...
pööning_esemed = ["hallitusseen", "lamp", "kalasnikov"]
...
# Generate world using seeded random
random.seed(SEED)  # Reset seed before generation to ensure consistency
for i in range(len(kohad)):
    # Create a copy of the item list to avoid modifying the original
    current_items = esemed_list[i].copy()
    randomhulk = random.randint(3, 10)
    ax = 0
    while ax != randomhulk:
        if len(current_items) == 0:
            break
        random_value = random.randint(0, len(current_items)-1)
        ese = current_items[random_value]
        
        if ese in ["liblikas", "karu", "lind", "hiir", "kalasnikov", "puhkpüss"] and eluka_lisamine > 0:
            eluka_lisamine -= 1
            current_items.pop(random_value)
        
        kohad[i].append(ese)
        ax += 1
    eluka_lisamine = 1

# Add ammo when weapons are present
for element in tiik:
    if element == "puhkpüss":
        tiik.append("nool_x20")
        break

for element in pööning:
    if element == "kalasnikov":
        pööning.append("kuulid_x20")
        break
...
#ränndamine----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- -------------    
    ...
    #shoot-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------        
        ...
        if weapon == "käead":
            ...
            for i, item in enumerate(tasku):
                if item[:8] == "nool_x":
                    tasku[i] = "nool_x" + str(int(item[8:]) - 1)
                    arvk = 1  
                
            if "nool_x0" in tasku:
                tasku.remove("nool_x0")
            if arvk == 0:
                kuulitu_teated = ["Pole kuule, mida lasta", "Kuulid puuduvad, mida lasta", "Relv ei tööta"]
                return random.choice(kuulitu_teated)
...
def send_message():
    global input_var, chat_area
    input_var = tk.StringVar()  # Changed from string to StringVar
...
def show_start_screen():
    start_root = tk.Tk()
    start_root.title("Rule-Based Chat IDE")
    ...
    # Create start button to break the loop
    def stop_program():
        start_root.destroy()  # This will close the window
        game_itself()  # Start the game

    def show_join_screen():
        start_root.withdraw()  # Hide main window
        join_root = tk.Toplevel()
        join_root.title("Join Game")
        join_root.geometry(f"{window_width}x{window_height}+{x}+{y}")

        # Create text box for join code
        join_code = tk.StringVar()
        join_entry = ttk.Entry(join_root, textvariable=join_code)
        join_entry.place(relx=0.5, rely=0.4, anchor="center")

        def save_and_close(event=None):
            global saved_code
            saved_code = join_code.get()
            join_root.destroy()
            start_root.destroy()
            game_itself()  # Start the game after joining

        # Bind Enter key to save_and_close
        join_entry.bind('<Return>', save_and_close)

        # Create join button
        join_button = tk.Button(join_root, text="JOIN", command=save_and_close)
        join_button.place(relx=0.5, rely=0.5, anchor="center")

    # Create start and join buttons
    start_button = tk.Button(start_root, text="START", command=stop_program)
    start_button.place(relx=0.4, rely=0.7, anchor="center")

    join_button = tk.Button(start_root, text="JOIN", command=show_join_screen)
